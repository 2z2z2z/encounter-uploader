...
- ☐ Переработать контрол-панель, а именно сами контрол-элементы, стандартизировать размеры элементов и их расстояний. Учесть, что сейчас есть "костыль" к элементу с чекбоксом "отрицательное", не допустить, чтобы не него наезжали соседние элементы (сейчас это учтено) или придумать как сделать более юзабильно, чтобы занимало меньше места по ширине. Никакого хардкода, компановка элементов должна быть универсальной
- ☐ Используй преимуществоенно PrimeVue и TailwindCSS, вместо кастомного html-кода и css-стилей. Не добавляй лишних классов, если это будет перегружать интерфейс (тени, бордеры, блоки)
