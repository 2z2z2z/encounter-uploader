...
- ☐ Убрать обертку <card></card> в LevelHeader.vue (сохранив title и subtitle в текущем виде или придумать более интересный вид, но обязательно из PrimeVue-компонентов, смотри документацию через context7)
- ☐ Переработать UI табов (LevelTabs), улучшить юзабилити и сделать компактнее (сейчас не очень удобно что на двух строчках, только на PrimeVue компонентах)
