...
- "Добавить коды": добавление в активный таб, дубликаты отбрасываются.
- "Очистить": очищает активный таб, подтверждение обязательно.
- Поле "Ответ": без нормализации и хоткеев.
- "Закрытие уровня": управляет `sectorMode`, не полем "Ответ".
- "Уровни бонуса": используем текущую логику, портированную на PrimeVue (множественный выбор).
