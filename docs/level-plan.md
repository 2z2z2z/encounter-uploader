...
### ☐ Шаг 30.5: Доработка страниц тестировани
...
- Удаление тестовых файлов (которы были нужны для тестирования единожды)
- Проверка комментариев на указание что что-то будет сделано на каком-то шаге (убедиться что такого нет и все сделано)
- Чистка комментариев от ненужной или излишней информации
- Проверить все todo, fallback, заглушки
- Добавление JSDoc комментариев к сложным участкам (если не хватает)
- Проверка что нет никакого хардкода типов уровней, все основывается на конфигах
- Проверка что в новой архитектуры не используются файлы старой архитектуры (и что старая архитектура готова у упразднению)
...
### ☐ Шаг 32: Улучшения
- Почему во многих местах используется CSS вместо классов PrimeVue или TailwindCSS?
- Проверка и избавление от HTML-кода в пользу PrimeVue-компонентов
- Исправление проблемы и ошибок из источников Vetur, ts-plugin

**Бизнес-цель**: Избавление от кастомного html и css кода, а также проблем и ошибок плагинов Vetur, ts-plugin для новой архитектуры.

**Критерии завершения:**
- ✅ Оптимизация и улучшения не поломали UI и не привели к ошибкам и проблемам, структура и функционал не затронуты
- ✅ План обновлен (чекбоксы проставлены, техническая реализация описана сухо, без воды и по фактам)

**Проверки:**
- ☐ ESLint без ошибок
- ☐ Vetur, ts-plugin без ошибок
- ☐ Сборка проекта без ошибок
- ☐ Тестирование UI с playwright при необходимости (Олимпийка на 15 секторов и 100500)

### ☐ Шаг 33: Переключение на новую архитектуру
- Переключить систему на использование новой архитектуры
...
- ☐ ESLint без ошибок
- ☐ Сборка проекта без ошибок
- ☐ Тестирование UI с playwright (с тестовыми данными для авторизации test/test) + проверка консоли браузера на наличие ошибок
