...
- Поддержка нескольких пресетов типов уровней: «Олимпийка» на 15/31/63/127 и тип «100500».
...
- `components/types/shared/Answers.vue` — единая таблица ответов/секторов/бонусов для «Олимпик».
...
- `components/types/shared/PageLayout.vue` — общий шаблон страницы с единым контейнером/заголовком (ширина `max-w-[120rem]`).
- `components/types/shared/ExportModal.vue`, `components/types/shared/PreviewModal.vue`, `components/types/shared/BottomBar.vue`, `components/types/shared/LevelInfo.vue` — переиспользуемые блоки.
- `components/types/olymp/OlympBase.vue` — единая база для всех «Олимпик»; `Olymp15/31/63/127` — тонкие обёртки.
- `composables/uploader.ts` — общий загрузчик: `uploadTask(html)`, `uploadSectors(rows)`, `uploadBonuses(answers)`.
- `components/types/olymp/useExportImportOlymp.ts` — экспорт/импорт «Олимпик» (JSON/CSV).
- `components/types/Type100500/useExportImport100500.ts` — экспорт/импорт `100500` (JSON/CSV, поддержка `delay`/`relativeLimit`).
- `types/upload.ts` — унифицированные модели данных: `Hms`, `UploadAnswer*`, `Row100500`, фабрики (`createHms`, `createEmptyAnswer`).
...
- UI:
  1) Создать компонент `components/types/<NewType>/index.vue` и обернуть содержимое в `shared/PageLayout`.
  2) Переиспользовать общие блоки: `shared/LevelInfo`, `shared/BottomBar`, при необходимости — `shared/PreviewModal`, `shared/ExportModal`, `shared/Answers`.
  3) Экспорт/импорт:
     - для «Олимпик» — `useExportImportOlymp`;
     - для `100500`‑подобных — `useExportImport100500`;
     - при другой схеме — собрать экспорт/импорт на базе `utils/csv`.
  4) Заливка: подключить общий `useUploader({ domain, gameId, levelId })` и передавать строки вида `{ number, variants[], inSector, closedText?, sectorName? }` и массив `Answer` для бонусов.
  5) Добавить пункт в селектор типа на `/settings` и обработку в `components/UploadForm.vue`.
  6) В `useUploadStore` настроить инициализацию количества строк/кеширования (`upload-<type>`).
  7) Если требуется другая раскладка «задания» — реализовать генератор вместо `utils/olymp.generateOlympLayout`.
