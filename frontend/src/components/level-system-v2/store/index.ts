 ...
 * 
 * Полностью независим от старого store.
...
interface StorageData {
	...
	tabs: TabData[]
	config: LevelV2StoreState['config']
	timestamp: number
}

/**
 * Основной store для level-system-v2
 */
export const useLevelV2Store = defineStore('level-v2', () => {
	...
	// ===== Конфигурация =====
	const config = ref<LevelV2StoreState['config']>({
		sectorMode: 'all',
		bonusTime: {
			hours: 0,
			minutes: 0,
	 ...
	 */
	function updateConfig(updates: Partial<LevelV2StoreState['config']>): void {
