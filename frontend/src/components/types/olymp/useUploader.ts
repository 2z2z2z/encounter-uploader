...
export function useUploader(ctx: UploadContext) {
	...
	async function uploadSectors(rows: Array<{ number: number; variants: string[]; inSector: boolean; closedText: string }>) {
		try {
			...
			for (const row of sectors) {
				...
				await sendSector(ctx.domain, ctx.gameId, ctx.levelId, row.variants, row.closedText)
			}
			progress.finish()
	...
	return { error, uploadTask, uploadSectors, uploadBonuses }
}
