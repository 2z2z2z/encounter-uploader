...
export function generateOlympLayout(total: number, level: string | number): Cell[][] {
  ...
  for (let r = 0; r < rows; r++) {
    layout[r].push({ id: `${level}_${pad(r + 1)}` })
  }
  let nextId = rows + 1
  for (let c = 1; c < k; c++) {
    const rowspan = 2 ** c
    for (let r = 0; r < rows; r += rowspan) {
      layout[r].push({ id: `${level}_${pad(nextId++)}`, rs: rowspan })
    }
  }
